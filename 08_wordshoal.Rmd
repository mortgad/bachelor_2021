...
```{r}
# Importing data
df <- read_csv("./data/folketinget_2009_2021_raw.csv") %>% mutate(
  month_year_id = paste0(Year, "_", month(Date)
))

df <- df %>% filter(month_year_id %in% c("2011_1", "2011_2", "2011_3", "2011_4"))
```
...
get_theta <- function(data){
  cat(paste0("[ ] Shoaling Month ", Month, ". Month: ", i, " out of ", length(df$Month), "\n"))
  corpus = quanteda::corpus(data)
  dfm <- dfm(tokens(corpus), remove_punct = TRUE, ignoredFeatures = stopwords) # stopwords("danish")
...
agg <- results %>% group_by(Parti, Date) %>% arrange(-overlap_score, .by_group = TRUE) %>% slice(1:3) %>% # rename df
  summarise(overlap_score = mean(overlap_score),
            difference_score = mean(difference_score)) 

agg <- left_join(x = agg, y = results %>% dplyr::select(Parti, Date, mean_theta, sd_theta), by = c("Parti" = "Parti", "Date" = "Date")) %>% group_by(Parti, Date) %>% slice(1) %>% ungroup() # Date might need to be changed to Month
...
write_csv(joined, "./data/wordshoal_results.csv") # check output file name
```
