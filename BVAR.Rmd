...
```{r}
df <- agg %>% filter(Parti == "Dansk Folkeparti") %>% select(Date, Inclusion, Differentiation)
```

```{r}
#checking out the outome variable
ggplot(df, aes(x = Inclusion)) + 
  geom_density()+
...
ggplot(df, aes(x = Differentiation)) + 
  geom_density()+
  labs(title = "Before Transformation")
...
df %>% group_by(Date) %>% pivot_longer(cols = c("Inclusion", "Differentiation"), names_to = "Need") %>% 
  ggplot()+
...
x <- fred_transform(x, codes = c(4,4))
