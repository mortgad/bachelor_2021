...
if data_to_be_used == 1:
	...
	if implementation == "1a":
		output_dir = "machine_learning_implementations/1a_estimates_ps/"

	if implementation == "1b":
		output_dir = "machine_learning_implementations/1b_estimates_ps/"
	...
	#set output location
	if implementation == "1a":
		output_dir = "machine_learning_implementations/1a_estimates_nps/"

	if implementation == "1b":
		output_dir = "machine_learning_implementations/1b_estimates_nps/"
...
c = csv.writer(open(output_dir+"sgd_scores_all_parties_1810_2015"+implementation+".csv", mode='w'))

if start_from == 1:
	c.writerow(["session_indicator","session_ref","score","best_alpha"])

c = csv.writer(open(output_dir+"sgd_scores_all_parties_1810_2015"+implementation+".csv", mode='a'))
	...
	if int(session_indicator) >= start_from:
		...
		for alpha in alphas:
			...
			for t in range(len(test_indices)):
				...
				if implementation in ["1b","2b"]:
					...
					gs_clf = Pipeline([('clf', SGDClassifier(n_jobs=-1,loss='log', penalty='l2', shuffle=True,alpha=alpha,class_weight = 'balanced',random_state=1234)),])
					...
					gs_clf = Pipeline([('clf', SGDClassifier(n_jobs=-1,loss='log', penalty='l2', shuffle=True,alpha=alpha,class_weight = None,random_state=1234)),])
		...
		for ref in range(len(all_test_targets)):
			...
			if implementation in ["1b","2b"]:
				...
				gs_clf = Pipeline([('clf', SGDClassifier(n_jobs=-1,loss='log', penalty='l2', shuffle=True,alpha=best_alpha,class_weight = 'balanced',random_state=1234)),])
				...
				gs_clf = Pipeline([('clf', SGDClassifier(n_jobs=-1,loss='log', penalty='l2', shuffle=True,alpha=best_alpha,class_weight = None,random_state=1234)),])
			...
			if ref == 0:
				individual_data.to_csv((output_dir+implementation+"_"+"sgd_individual_estimates_"+str(session_ref)+".csv"), sep=',')
			else:
				individual_data.to_csv((output_dir+implementation+"_"+"sgd_individual_estimates_"+str(session_ref)+".csv"), sep=',', mode = 'a', header=False)
			print("Entered data: implementation"+"_"+"sgd_individual_estimates_"+str(session_ref)+".csv")
