...
#replace all "_ps" with "_nppr" in this script to run version with data that includes procedural phrases
implementation <- "ps" 
...
library(rjags)
source("poisson_model_implementations/3.wordshoal/wfms_ps/Wordshoal9.R")  
...
fileNames <- Sys.glob("poisson_model_implementations/3.wordshoal/wfms_ps/*.RData")
...
################################
 ...
 #for aggregation by titles (wfm04):
 #debateData <- aggregate(debateData$speakersN,by=list(debateData$session_ref,debateData$title),sum)
...
if (fullrun){
  ...
  save(wordseine.out,file="poisson_model_implementations/3.wordshoal/wfms_ps/Estimates/WordSeineUK.Rdata")
  save(wordfish.details,file="poisson_model_implementations/3.wordshoal/wfms_ps/Estimates/WordfishUK.Rdata")
  ...
  load("poisson_model_implementations/3.wordshoal/wfms_ps/Estimates/WordSeineUK.Rdata")
  load("poisson_model_implementations/3.wordshoal/wfms_ps/Estimates/WordfishUK.Rdata")	
}	
...
'
inits = list(theta = rnorm(N,0,1), alpha=rnorm(M,0,4), beta = rnorm(M,0,4), tau = rgamma(N,1,1),
  .RNG.name="base::Super-Duper", .RNG.seed=42)
...
if (fullrun){
  ...
  save(posterior,file="poisson_model_implementations/3.wordshoal/wfms_ps/Estimates/WordShoalUK.Rdata")
} else {
  load("poisson_model_implementations/3.wordshoal/wfms_ps/Estimates/WordShoalUK.Rdata")
}	
...
labels <- data.frame(debateSpeakerData$speakerID,debateSpeakerData$speaker.name,debateSpeakerData$session,as.character(debateSpeakerData$party))
colnames(labels) <- c("legislator","speaker.name","session","party")

relevant_ids <- data.frame(legislator = as.numeric(levels(wordseine.out$Legislator)))

labels <- unique(merge(relevant_ids,labels,by='legislator'))
...
write.csv(final_wordseine_results,paste("poisson_model_implementations/UK_wordfish_estimates_",implementation,".csv",sep=""))

final_results <- do.call(rbind.data.frame, all_results)
write.csv(final_results,paste("poisson_model_implementations/UK_wordshoal_",implementation,".csv",sep=""))
}
}

log_file <- file(logfile_name, open = "a")
cat(paste("\n",paste("UK_wordfish_estimates_",implementation,".csv",sep="")," successfully generated at: ",Sys.time(),". (location: ",paste("poisson_model_implementations/UK_wordfish_estimates_",implementation,".csv",sep=""),sep=""), file = log_file,append = TRUE)
close(log_file)

log_file <- file(logfile_name, open = "a")
cat(paste("\n",paste("UK_wordshoal_",implementation,".csv",sep="")," successfully generated at: ",Sys.time(),". (location: ",paste("poisson_model_implementations/UK_wordshoal_",implementation,".csv",sep=""),sep=""), file = log_file,append = TRUE)
close(log_file)
