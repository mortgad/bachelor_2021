...
```{r}
df %>% group_by(month_year_id) %>% count() %>% arrange(n)

subset <- df %>% filter(month_year_id == "2019_8") %>% group_by(Parti) %>% filter(n() > 1) %>% ungroup() 
...
jep <- left_join(heppe, agg, by = c("From" = "Parti", "month_year_id" = "month_year_id")) %>% 
  rename(
    Parti = From,
    overlap_with = To,
  ...
  select(month_year_id, Parti, overlap_with, overlap_score, difference_score, mean_theta, sd_theta)

write_csv(results, "./data/agg_results_2009_2021.csv")
```
